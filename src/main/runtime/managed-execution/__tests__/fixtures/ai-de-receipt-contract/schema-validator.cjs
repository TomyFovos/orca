...
function resolveReference(root, reference) {
  if (!reference.startsWith('#/')) {
    throw new SchemaValidationError('/$ref');
  }
...
function assertAlternatives(value, alternatives, root, pointer, exactOne) {
  ...
  if (matches === 0 || (exactOne && matches !== 1)) {
...
function validateNode(value, schema, root, pointer) {
  ...
  if (schema.$ref) {
    return validateNode(value, resolveReference(root, schema.$ref), root, pointer);
  }
  if (schema.oneOf) {
    assertAlternatives(value, schema.oneOf, root, pointer, true);
  }
  if (schema.anyOf) {
    assertAlternatives(value, schema.anyOf, root, pointer, false);
  }
  for (const child of schema.allOf || []) {
    validateNode(value, child, root, pointer);
  }
  ...
  if (Array.isArray(value)) {
    validateArray(value, schema, root, pointer);
  } else if (value !== null && typeof value === 'object') {
    validateObject(value, schema, root, pointer);
  } else {
    validateScalar(value, schema, pointer);
  }
  return true;
}
