...
export function listRelayBaseDirsCommand(host: RemoteHostPlatform, baseDir: string): string {
  ...
  return powerShellCommand(
    [
      ...
      "Get-ChildItem -LiteralPath $base -Directory -Filter 'relay-*' -ErrorAction Stop | Where-Object { $_.Name -match '^relay-(v?[0-9]+\\.[0-9]+\\.[0-9]+(\\+[0-9a-f]+)?)(\\.gc-tombstone\\.[0-9]+\\.[0-9]+)?$' } | Sort-Object Name | Select-Object -First " +
